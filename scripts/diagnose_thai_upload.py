...
Or copy-paste the content into odoo.sh shell
"""

import odoo
from odoo import api, SUPERUSER_ID

# For odoo.sh shell, the env is already available
# For local shell, uncomment and set your database name:
...
# 4. Check how statements are created - test the actual method
print("\n\n4. TESTING STATEMENT CREATION METHOD")
print("-" * 70)

if journals:
    # Find a journal with default_account_id set
    configured_journal = None
    for j in journals:
        if j.default_account_id:
            configured_journal = j
            break
    
    if configured_journal:
        print(f"\nTesting with properly configured journal: {configured_journal.name}")
        print("Simulating statement creation (DRY RUN - no actual data created)...")
        
        # Check if the journal can handle statement creation
        try:
            # Don't actually create, just check the method exists
            print(f"✅ Journal has _create_bank_statements method: {hasattr(configured_journal, '_create_bank_statements')}")
            print(f"✅ Default account is set: {configured_journal.default_account_id.code}")
        except Exception as e:
            print(f"❌ Error: {e}")
    else:
        print("\n⚠️  No journals with default_account_id found - cannot test creation")
...
# 6. Check wizard model and access
print("\n\n6. CHECKING WIZARD MODEL")
print("-" * 70)
